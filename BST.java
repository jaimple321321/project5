...
public class BST<E extends Comparable<? super E>> implements Iterable<E> {
    ...
    private static class InOrderIterator<E> implements Iterator<E> {
        private final Deque<Node<E>> st = new ArrayDeque<>();
        InOrderIterator(Node<E> root) { pushLeft(root); }
    ...
    private static class PreIterator<E> implements Iterator<E> {
        private final Deque<Node<E>> st = new ArrayDeque<>();
        PreIterator(Node<E> root) { if (root != null) st.push(root); }
    ...
    private static class PostIterator<E> implements Iterator<E> {
        private final Deque<Node<E>> out = new ArrayDeque<>();
        PostIterator(Node<E> root) {
            if (root == null) return;
            Deque<Node<E>> st = new ArrayDeque<>();
            st.push(root);
            while (!st.isEmpty()) {
        ...
        @Override
        public boolean hasNext() { return !out.isEmpty(); }
